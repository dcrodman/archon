...
func (s *PatchServer) handlePatchLogin(ctx context.Context, c *Client) error {
	if err := SendPatchDataAck(ctx, c); err != nil {
		return err
	}
	if err := SendPatchFileList(ctx, c, rootNode); err != nil {
		return err
	}
	return SendPatchFileListDone(ctx, c)
}

// Acknowledgement sent after the DATA connection handshake.
func SendPatchDataAck(ctx context.Context, c *Client) error {
	return c.Send(ctx, commands.PCHeader{Type: commands.PatchDataAckType, Size: 0x04})
}

// Traverse the patch tree depth-first and send the check file requests.
func SendPatchFileList(ctx context.Context, c *Client, node *directoryNode) error {
	if err := SendPatchChangeDir(ctx, c, node.clientPath); err != nil {
		return err
	}

	for _, patch := range node.patchFiles {
		if err := SendPatchCheckFile(ctx, c, patch.index, patch.filename); err != nil {
			return err
		}
	}

	for _, subDirectory := range node.childNodes {
		if err := SendPatchFileList(ctx, c, subDirectory); err != nil {
			return err
		}
		// Move them back up each time we leave a directory.
		if err := SendPatchDirAbove(ctx, c); err != nil {
			return err
		}
	}
...
func SendPatchChangeDir(ctx context.Context, c *Client, dir string) error {
...
func SendPatchCheckFile(ctx context.Context, c *Client, index uint32, filename string) error {
...
func SendPatchDirAbove(ctx context.Context, c *Client) error {
	return c.Send(ctx, commands.PCHeader{Type: commands.PatchDirAboveType, Size: 0x04})
}

// Tell the client that we've finished sending the patch list.
func SendPatchFileListDone(ctx context.Context, c *Client) error {
	return c.Send(ctx, commands.PCHeader{Type: commands.PatchFileListDoneType, Size: 0x04})
}
...
func (s *PatchServer) handleFileStatus(ctx context.Context, c *Client, fileStatus *commands.PatchFileStatus) {
...
func updateClientFiles(ctx context.Context, c *Client) error {
	...
	if numFiles > 0 {
		if err := SendPatchStartFileUpdate(ctx, c, numFiles, totalSize); err != nil {
			return err
		}
		if err := traverseAndUpdate(ctx, c, rootNode); err != nil {
			return err
		}
	}

	return SendPatchUpdateComplete(ctx, c)
}

// Send the total number and cumulative size of files that need updating.
func SendPatchStartFileUpdate(ctx context.Context, c *Client, num, totalSize uint32) error {
	return c.Send(ctx, commands.PatchStartFileUpdate{
...
func traverseAndUpdate(ctx context.Context, c *Client, node *directoryNode) error {
	if err := SendPatchChangeDir(ctx, c, node.name); err != nil {
		return err
	}

	for _, file := range node.patchFiles {
		if entry, ok := c.FilesToUpdate[int(file.index)]; ok {
			if err := updateClientFile(ctx, c, entry.(*fileEntry)); err != nil {
				return err
			}
		}

		for _, dir := range node.childNodes {
			if err := traverseAndUpdate(ctx, c, dir); err != nil {
				return err
			}
		}
	}
	...
	return SendPatchDirAbove(ctx, c)
}

func updateClientFile(ctx context.Context, c *Client, patch *fileEntry) error {
	if err := SendPatchFileHeader(ctx, c, patch); err != nil {
		return nil
	}
	...
	for i := 0; i < nChunks; i++ {
		...
		if err := SendPatchFileChunk(ctx, c, uint32(i), checksum, uint32(bytes), chunkBuf); err != nil {
			return err
		}
	}

	return SendPatchFileComplete(ctx, c)
}

// send the header for a file we're about to update.
func SendPatchFileHeader(ctx context.Context, c *Client, patch *fileEntry) error {
...
func SendPatchFileChunk(ctx context.Context, c *Client, chunk, chksm, chunkSize uint32, fdata []byte) error {
	...
	return c.Send(ctx, commands.PatchFileChunk{
...
func SendPatchFileComplete(ctx context.Context, c *Client) error {
	return c.Send(ctx, commands.PCHeader{Type: commands.PatchFileCompleteType, Size: 0x04})
}

// We've finished updating files.
func SendPatchUpdateComplete(ctx context.Context, c *Client) error {
	return c.Send(ctx, commands.PCHeader{Type: commands.PatchUpdateCompleteType, Size: 0x04})
}
