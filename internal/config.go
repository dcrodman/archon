...
const configEnvVarPrefix = "ARCHON"

// Config is a global reference to the server's configuration.
var Config = &archonConfig{}

// InitConfig initializes Viper with the contents of the config file under configPath.
func InitConfig(configPath string) {
	...
	for _, k := range viper.AllKeys() {
		...
		if err := viper.BindEnv(k, configEnvVarPrefix+"_"+envVar); err != nil {
			fmt.Printf("error binding %s to %s", k, configEnvVarPrefix+"_"+envVar)
			os.Exit(1)
		}
	}
...
// archonConfig contains all of the configuration options available to any of Archon's
...
func (c *archonConfig) DatabaseURL() string {
	return fmt.Sprintf(
...
func (c *archonConfig) BroadcastIP() [4]byte {
...
func (c *archonConfig) QualifiedPath(files ...string) string {
