...
// BlowfishCipher is an instance of Blowfish encryption using a particular key.
...
func newBlowfishCipher(key []byte) *BlowfishCipher {
	...
	if k := len(key); k < 1 || k > 56 {
		panic(fmt.Sprintf("crypto/blowfish: invalid key size %d", k))
	}
...
func (c *BlowfishCipher) Encrypt(data []byte, size uint32) {
	for i := 0; i < int(size); i += BlowfishBlockSize {
		c.encryptBlock(data[i : i+BlowfishBlockSize])
	}
}

func (c *BlowfishCipher) encryptBlock(data []byte) {
	if len(data) > 8 {
		panic("encrypt must be called on 8-byte blocks")
	}

	l := toLittleEndian(data[0:4])
	r := toLittleEndian(data[4:8])
	l, r = encryptData(l, r, c)
	data[0], data[1], data[2], data[3] = byte(l), byte(l>>8), byte(l>>16), byte(l>>24)
	data[4], data[5], data[6], data[7] = byte(r), byte(r>>8), byte(r>>16), byte(r>>24)
}
...
// Decrypt decrypts the 8-byte buffer src using the key k and stores the result in data.
func (c *BlowfishCipher) Decrypt(data []byte, size uint32) {
	for i := 0; i < int(size); i += BlowfishBlockSize {
		c.decryptBlock(data[i : i+BlowfishBlockSize])
	}
}

func (c *BlowfishCipher) decryptBlock(data []byte) {
	if len(data) > 8 {
		panic("decrypt must be called on 8-byte blocks")
	}
